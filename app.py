...
function renderServer(){
 ...
 mainArea.innerHTML=`<header class="topbar"><div><div class="topTitle">${esc(s.name)}</div><div class="topSub">${s.member_count} member${s.member_count===1?"":"s"}</div></div><div class="topActions">${notificationBellButton()}${s.my_role==="owner"?`<button class="ghost" onclick="showServerSettings(${s.id})">⚙ Server settings</button>`:""}<button class="ghost" onclick="toggleMembers()">👥 Members</button></div></header>
...
async function staffUserSearch(){const q=staffSearch.value.trim();if(!q)return;try{const d=await api("/api/staff/users?q="+encodeURIComponent(q));staffUsers.innerHTML=d.users.map(u=>`<div class="listItem"><img class="avatar" src="${avatarSrc(u.avatar)}" onerror="this.style.visibility='hidden'"><div class="listMain"><div class="listTitle">${esc(u.username)} <span class="pill">${esc(u.global_role)}</span></div><div class="listSub">${esc(u.email)} · ID ${u.id}${u.banned?" · BANNED":""}</div></div><button class="ghost" onclick="staffUserActions(${u.id})">Manage</button></div>`).join("")||"<div class='muted'>No users found.</div>"}catch(e){toast(e.message)}}
